...
const ManageCoupons = (props) => {
    ...
    const fetchList = () => {
        ...
        axios.get(Utils.apiUrl('coupons/list'), Utils.apiHeaders())
            .then(res => {
                if (res.data.success) {
    ...
    if(window.confirm("You are going to delete record, are you sure?")){
        axios.delete(Utils.apiUrl(`coupons/${id}`),Utils.apiHeaders())
        .then(res => {
            fetchList();
                    ...
                    <Tab.Container id="left-tabs-example">
                        <div className="card-header ui-sortable-handle" >
                            ...
                            <span className="btn float-right">
                                <button className="btn btn-success btn-sm" onClick={() => setShowForm({ mode: 1, id: false })}>Add Coupon <i className="fas fa-plus"></i></button>
                            </span>
                        </div>
                        <Row>
                            <Col sm={12}>
