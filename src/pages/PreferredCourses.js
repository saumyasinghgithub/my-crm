...
const PreferredCourses = (props) => {
  ...
  const removeFav = (course_id) => (e) => {
    setServerData("course/markfav", `course_id=${course_id}&fav=0`, "post").then(fetchFavorites);
  };
    ...
    return (
      <>
        <Container>
          <Row>
            {favs.data.map((rec) => (
              <Col className="col-lg-4 col-md-6 col-12 py-1">
                <Card className="courseWrapper coursecard my-2 h-100">
                  <Card.Body>
                    <Row>
                      <Col className="col-12">
                        <div
                          className="imgWrapper rounded"
                          style={{ backgroundImage: `url("${process.env.REACT_APP_API_URL}/uploads/courses/${rec.course_image}")` }}
                        ></div>
                      </Col>
                      <Col className="col-12">
                        <h3>{rec.name}</h3>
                    ...
                    <Row>
                      <Col className="col-4 text-left mt-2">
                        <button className="btn text-white bg-danger" onClick={removeFav(rec.id)}>
                          Remove
                        </button>
                      </Col>
                      <Col className="col-8 text-right mt-2">
                        <a href={`${process.env.PUBLIC_URL}/courses/${rec.slug}`} className="btn btnBlue preferdCourseBtn">
                          View Details
                        </a>
                      </Col>
                    </Row>
      ...
      <Container className="h-100 p-0">
        <div className="profile-wrapper">
          <div className="container">
            {_.get(props, "sitesetting.preferred_courses", 1) === 0 && <h1>Invalid Access</h1>}
            {_.get(props, "sitesetting.preferred_courses", 0) > 0 && (
              <>
