...
const LibraryForm = (props) => {
  ...
  const onSave = (e) => {
    ...
    console.log(frmdata);
    setSaving(true);
    setServerData("trainer/my-library", frmdata).then((res) => {
      setSaving(false);
      setResponse(res);
    });
  };
    ...
    return (
      <>
        ...
        <div className="text-center">
          {!_.isEmpty(_.get(mylibrary, fld + "_image", "")) && (
            <img src={`${process.env.REACT_APP_API_URL}/uploads/${fld}/${mylibrary[fld + "_image"]}`} className="thumbnail mt-3" />
          )}
        </div>
      </>
    );
      ...
      <Row>
        ...
        <Col md={9} className="mt-3">
          ...
          <Editor
            ...
            toolbar="undo redo | bold italic underline strikethrough | fontselect | fontsizeselect | code | formatselect | alignleft aligncenter alignright alignjustify | outdent indent |  numlist bullist | forecolor backcolor removeformat | pagebreak | charmap emoticons | fullscreen  preview save print | insertfile image media template link anchor codesample | ltr rtl"
            plugins="code"
