...
const LoginModal = (props) => {
    ...
    const [fPassing, setFPassing] = useState({loading: false, success: true, message: ""});
    ...
    <div className="modal-dialog modal-full" >
        <div className="modal-content">
                ...
                {fPassing.loading===true && <div className="alert alert-info p-5 m-5">
                    Sending your request to reset password...
                    <div className="progress">
                        <div className="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" aria-valuenow="100" style={{"width": "100%"}}></div>
                    </div>
                </div>}
                {!logining && ! fPassing.loading && <div className="loginWrapper">
                    {mode===1 && <h3>Log In to see <br /> the latest updates</h3>}
                    {mode===2 && <><h3>Forgot your password?</h3><h4>Fill your registered email with us to reset it!</h4></>}
                    ...
                    {fPassing.message!=="" && <div className={`alert alert-${fPassing.success ? "warning" : "danger"} p-5 m-5`}>
                        {fPassing.message}
                    </div>}
                    
                    {fPassing.message==='' && <form onSubmit={mode===1 ? onLogin: onFPass} className="needs-validation" noValidate>
                    ...
                    {mode === 1 && <>
                        <p>Forgot password ? <a href="" onClick={toogleMode}>Click here!</a></p>
                    ...
                    {mode === 2 && <p><a href="" onClick={toogleMode}>Click here</a> to Login!</p>}
                </div>}           
        </div>
    </div>
</div>
