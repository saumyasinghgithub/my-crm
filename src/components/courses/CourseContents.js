...
const CourseContents = (props) => {
  ...
  const deleteRecord = (id) => (e) => {
    if (window.confirm("You are going to delete record, are you sure?")) {
      axios.delete(Utils.apiUrl(`trainer/course-content/${id}`), apiHeaders()).then((res) => {
        fetchList();
  ...
  const fetchList = () => {
    ...
    axios.get(Utils.apiUrl(`trainer/course-content/?${params}`), apiHeaders()).then((res) => {
      if (res.data.success) {
    ...
    <div className="card-header ui-sortable-handle">
      ...
      <span className="btn float-right">
        <button className="btn btn-success btn-sm" onClick={() => setShowForm({ mode: 1, id: false })}>
          Add Course Content <i className="fas fa-plus"></i>
        </button>
      </span>
    </div>
  );
  ...
  return (
    <Modal show={true} size="xl" onHide={_.get(props, "onClose", "")}>
      ...
      {showForm.mode > 0 && (
        <ContentForm type="modal" id={showForm.id} course_id={props.id} onClose={() => setShowForm({ ...showForm, mode: 0 })} onSave={fetchList} />
      )}
    </Modal>
  );
};
