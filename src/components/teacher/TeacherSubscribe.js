...
const TeacherSubscribe = (props) => {
    ...
    const onSave = (event) => {
        ...
        axios.post(Utils.apiUrl('trainer/subscribers'), frmdata, Utils.apiHeaders({ token: token }))
            .then(res => {
                if (res.data.success) {
    ...
    return (<div>
        {inLine ? (
                        ...
                        <form noValidate ref={frmRef} id="subscription-form" onSubmit={onSave} method="post" className="form contact alertdesign needs-validation">
                            <div className="row">
                                <div className="col-md-5 col-md-offset-3 contactSubs">
                                    ...
                                    <input name="email" id="email" placeholder="Please enter your email here for subscription" title="Email" className="input-text form-control landingpageInput p-3" type="email" required />
                                    <div className="invalid-feedback">Email address is required!</div>
                                    <p><b>{showMessage}</b></p>
                                </div>
                                <div className="col-md-2 col-md-offset-2 contactSubs">
                                    <div class="HomeRegister SubmitHomeSubscribe"> <Button color="secondary" type="submit">SUBSCRIBE</Button></div>
                                </div>
                            </div>
                        </form>
                    </div>
            ...
            <Modal isOpen={modal} toggle={toggle}>
                ...
                <form noValidate id="subscription-form-modal" ref={frmRef} onSubmit={onSave} method="post" className="form contact alertdesign needs-validation">
                    <ModalBody>
